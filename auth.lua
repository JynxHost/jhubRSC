-- Plato configuration
local accountId = 38660; -- Plato account id [IMPORTANT]
...
local localPlayerId = game:GetService("RbxAnalyticsService"):GetClientId();
local rateLimit, rateLimitCountdown, errorWait = false, 0, false;
-- Plato internals [END]

-- Plato global functions [START]
local function getLink()
    return fStringFormat("https://gateway.platoboost.com/a/%i?id=%s", accountId, localPlayerId);
end;
